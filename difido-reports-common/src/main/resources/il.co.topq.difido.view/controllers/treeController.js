    ...
    $(source).each(function() {
        switch (this.type) {
            case "scenario" :
                ...
                break;
            case "test":
                destination.push({'text': this.name, 'icon': testIcon(this.status), 'rel': this.status, 'type': this.status, 'a_attr': {'href': "tests/test_" + this.uid+"/test.html",'class':'s_' + this.status + "_text"}});
                break;
        }
    });
}
...
function treeController(element) {
    var json = execution;
    var tree = {'text': 'Difido', 'icon': 'images/jsystem_ico.gif', 'children': []};
    $(json.machines).each(function(machineIndex) {
        tree.children.push({'text': this.name, icon: 'images/machine.png', 'children': [], 'state': {'opened': true, 'selected': true}, 'a_attr':{'class' : 's_' + this.status +"_text"}});
        $(this.children).each(function(scenarioIndex) {
            var children = new Array();
            tree.children[machineIndex].children[scenarioIndex] = {'text': this.name, icon: suiteIcon(this.status), 'children': children, 'a_attr':{'class' : 's_' + this.status +"_text"}};
            populateChildren(this.children, children);
        });
